...
async function routeDistances(origin,places){
  ...
  const url='https://router.project-osrm.org/route/v1/foot/'+coords+'?overview=false&steps=false';
  try{
    const r=await fetch(url);
    if(!r.ok)return [];
    const j=await r.json();
    // OSRM route individual para cada destino: a tabela pública pode não oferecer foot em todos os nós.
    const out=[];
    for(const p of places){
      try{
        const rr=await fetch('https://router.project-osrm.org/route/v1/foot/'+origin.lon+','+origin.lat+';'+p.lon+','+p.lat+'?overview=false&steps=false');
        if(!rr.ok)continue;
        const jj=await rr.json();
        const d=jj?.routes?.[0]?.distance;
        if(Number.isFinite(Number(d)))out.push({...p,distance:Number(d)});
      }catch(_){}
    }
    return out;
  }catch(_){return [];}
