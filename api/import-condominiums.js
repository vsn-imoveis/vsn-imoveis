  ...
  try{
    ...
    for(let page=0;page<maxPages;page++){
      ...
      const payload=rows.map(x=>{
        ...
        return {normalized_name:norm,name,legal_name:x.razao_social||null,trade_name:x.nome_fantasia||null,cnpj,address:x.logradouro||null,number:x.numero||null,complement:x.complemento||null,neighborhood:x.bairro||null,city:"São Paulo",state:x.uf||"SP",cep:x.cep||null,cnae:x.cnae_fiscal||null,cnae_description:x.cnae_fiscal_descricao||null,status:x.descricao_situacao_cadastral||x.situacao_cadastral||null,opened_at:x.data_inicio_atividade||null,phone:[x.ddd_telefone_1,x.ddd_telefone_2].filter(Boolean).join(" / ")||null,email:x.email||null,legal_nature:x.natureza_juridica||null,capital_social:x.capital_social||null,confidence:"cnpj",source_urls:[{title:"Minha Receita / CNPJ",url:"https://minhareceita.org/"+cnpj}],updated_at:new Date().toISOString()};
