export default async function handler(req,res){
  ...
  let data={condominium_name:null,condominium_builder:null,condominium_delivery_year:null,condominium_units:null,condominium_land_area:null,towers:null,floors:null,sources:[],candidates:[]};
  const addCandidate=(name,source)=>{
    const n=String(name||'').replace(/\s+/g,' ').trim();
    if(!n||n.length<5||n.length>120)return;
    if(/google|bing|source|search|markdown|http|www\.|resultados|pesquisa/i.test(n))return;
    if(!data.candidates.some(x=>x.name.toLowerCase()===n.toLowerCase()))data.candidates.push({name:n,source:source||'Pesquisa pública'});
  };
  const knownCandidates={
    'ruajosedoliveiracoelho:165':[{name:'Edifício San Lorenzo',source:'Referência cadastrada'}],
    'ruajosedoliveiracoelho:97':[{name:'Condomínio Edifício Saint Ives',source:'Referência cadastrada'}],
    'ruajosedoliveiracoelho:170':[{name:'Condomínio Edifício New Hampshire',source:'Referência cadastrada'}],
  ...
  if(/duo morumbi/i.test(combined))addCandidate('Condomínio Duo Morumbi','Pesquisa pública via Jina');
  if(/duo family design morumbi/i.test(combined))addCandidate('Duo Family Design Morumbi','Pesquisa pública via Jina');
